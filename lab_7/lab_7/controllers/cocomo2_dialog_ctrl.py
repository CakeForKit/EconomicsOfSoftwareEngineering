...
class Cocomo2Dialog(QDialog):
    ...
    def compose_run(self):
        ...
        windows_count = sum(screen_counts) + sum(report_counts)
        labor_costs, time, avg_workers, money, p = ApplicationCompositionModel.process(ruse, exp_idx, screen_counts, report_counts, third_gen_land, p, month_cost)
        self.__show_comp_res(labor_costs, time, avg_workers, money, p, windows_count)

    def __show_comp_res(self, labor_costs, time, avg_workers, money, p, windows_count):
        self.ui.comp_p_label.setText(str(p))
        self.ui.comp_windows_label.setText(str(windows_count))
    ...
    def arch_run(self):
        ...
        self.__show_arch_res(labor_costs, time, avg_workers, money, p)

    def __show_arch_res(self, labor_costs, time, avg_workers, money, p):
        self.ui.early_p_label.setText(str(p))
